...
const UserCard: React.FC<UserCardProps> = ({ username, isWaiting = false }) => {
    return (
        <div className={`flex p-2 ${isWaiting ? 'bg-grey-50 opacity-40' : 'bg-grey-50'} shadow-md h-[60px] w-[340px] rounded-[16px] drop-shadow-lg`}>
            <div className="flex items-center justify-center mr-4">
                <Icon name="woman_avatar"/>
            </div>
            <div className="flex flex-col justify-center items-start flex-1 text-lg font-semibold text-gray-900 text-center font-roboto text-[14px]">
                {username}
            </div>
